...
def get_gemini_response(prompt, image=None):
    model = genai.GenerativeModel('gemini-pro-vision') if image else genai.GenerativeModel('gemini-pro')
    response = model.generate_content(prompt if not image else [prompt, image])
    return response.text
# Concatenate the parts of the response
    response_text = ''.join([part.content for part in response.candidates[0].content.parts])
...
def display_ats_section():
    ...
    job_description = st.text_area("Job Description", help="Provide a detailed job description for evaluation.", placeholder="Enter the job description here...", height=200)
    uploaded_file = st.file_uploader("Upload Your Resume (PDF)", type="pdf", help="Upload your resume in PDF format for evaluation.", accept_multiple_files=False)
    ...
    if submit:
        if uploaded_file is not None:
            with st.spinner("Evaluating resume..."):
                ...
                response = get_gemini_response(input_prompt.format(text=text, jd=job_description))
...
def main():
    ...
    with st.sidebar:
        st.title("GenAI")
        nav_options = ["Gen-AI Chatbot", "Gen-AI Image-based Q&A", "Gen-AI YouTube Video Summarizer", "Gen-AI Text Summarizer", "Gen-AI Smart ATS", "Gen-AI Chat with PDF"]
    ...
    with st.container():
        ...
        if selected_page == "Gen-AI Chatbot":
            input_text = st.text_input("Ask me anything:", help="Enter your question for the chatbot.", placeholder="Type your question here...", key="chatbot_input")
            ...
            uploaded_image = st.file_uploader("Upload an image", type=["jpg", "jpeg", "png"], accept_multiple_files=False, help="Upload an image for analysis.", disabled=False, label_visibility="visible")
            image_prompt = st.text_input("Question about the image:", help="Ask a question related to the uploaded image.", placeholder="Type your question here...", key="image_input")
            submit_image_button = st.button("Tell me about the image", use_container_width=True)
            ...
            youtube_link = st.text_input("Enter YouTube video link:", help="Provide the link to the YouTube video for summarization.", placeholder="Enter YouTube video link...", key="youtube_input")
            ...
            long_text = st.text_area("Paste your long article or blog post here:", help="Paste the text you want to summarize.", placeholder="Paste your text here...", height=300, key="text_input")
            summarize_text_button = st.button("Summarize Text", use_container_width=True)
            ...
            pdf_docs = st.file_uploader("Upload your PDF files", accept_multiple_files=True, help="Upload PDF files for chatbot interaction.", disabled=False, label_visibility="visible")
            ...
            user_question = st.text_input("", help="Ask a question related to the content of the uploaded PDF files.", placeholder="Type your question here...", key="pdf_input")
            if user_question:
                with st.spinner("Generating response..."):
